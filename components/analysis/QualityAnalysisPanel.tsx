  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-50 flex items-center justify-center p-6">
      ...
      >
        ...
        {previousAttemptCount > 0 && !isLoading && (
          <div className="px-6 py-2.5 border-b border-amber-500/20 bg-amber-500/5 flex items-center gap-2">
            <Zap className="w-3.5 h-3.5 text-amber-400 shrink-0" />
            <span className="text-xs text-amber-300">
              <span className="font-bold">Self-correcting</span> — {previousAttemptCount} previous fix{previousAttemptCount > 1 ? 'es' : ''} applied. Suggesting different approaches.
            </span>
          </div>
        )}
                  ...
                  >
                    ...
                    {isExpanded && (
                      <div className="px-4 pb-4 pt-0 border-t border-slate-800/50">
                        ...
                        {issue.prompt_optimization_tips && issue.prompt_optimization_tips.length > 0 && (
                          <div className="mb-3 p-2.5 bg-slate-800/50 rounded-lg border border-slate-700/50">
                            ...
                            <ul className="space-y-1">
                              {issue.prompt_optimization_tips.map((tip, tipIdx) => (
                                <li key={tipIdx} className="text-[11px] text-slate-400 flex items-start gap-1.5">
