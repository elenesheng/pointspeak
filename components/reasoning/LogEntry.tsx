...
export const LogEntry: React.FC<LogEntryProps> = ({ log, onForceExecute, onAlternativeClick }) => {
  const getIcon = () => {
    switch(log.type) {
      ...
      case 'action': return <Target className="w-4 h-4 text-indigo-400" />;
    ...
    <div className="flex gap-3 animate-in fade-in slide-in-from-bottom-2 duration-300 group">
      ...
      <div className="flex-1">
        ...
        {log.type === 'analysis' && log.metadata ? (
             <div className="bg-slate-800/50 border border-indigo-500/10 rounded-xl p-4 shadow-sm mt-1">
             ...
             <div className="space-y-4">
               <div>
                 <p className="text-xs font-bold text-indigo-400/80 uppercase tracking-wide mb-1">Identified Room</p>
                 <p className="text-sm font-medium text-slate-200">{log.metadata.room_type}</p>
               </div>
               <div>
                   ...
                   {log.metadata.constraints.map((c: any, i: number) => (
                     <div key={i} className="bg-slate-900/50 p-2.5 rounded-lg border border-slate-800/50">
                       <div className="flex justify-between items-start mb-1">
                         ...
                         <span className="text-xs text-slate-500 bg-slate-800 px-1.5 py-0.5 rounded">{c.location}</span>
                       </div>
               ...
               <div>
                 <p className="text-xs font-bold text-indigo-400/80 uppercase tracking-wide mb-1 flex items-center gap-1">
                   <Activity className="w-3 h-3" />
                   Traffic Flow
                 </p>
                 <p className="text-sm text-slate-400 leading-relaxed italic border-l-2 border-indigo-500/20 pl-3 py-1">
            ...
            <div className={`rounded-xl p-4 shadow-sm border bg-slate-800/50 border-slate-700/50 mt-1`}>
              <h4 className="text-sm font-bold mb-2 flex items-center gap-2 text-slate-200">
                {log.metadata.operation_type} OPERATION
              </h4>
              <p className="text-sm text-slate-300 mb-4">{log.content}</p>
              <div className="bg-slate-950/30 p-3 rounded-lg border border-white/5">
                <p className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">Execution Plan</p>
                <p className="text-sm text-slate-300 leading-relaxed">{log.metadata.proposed_action}</p>
              </div>
            </div>
        ) : log.type === 'validation' && log.metadata && !log.metadata.valid ? (
            <div className="rounded-xl p-4 border shadow-sm bg-amber-500/5 border-amber-500/20 mt-1">
               <h4 className="text-sm font-bold mb-2 flex items-center gap-2 text-amber-300">
                  🚫 Action Blocked
               </h4>
               <p className="text-xs text-amber-200/80 mb-3 leading-relaxed">
                  The requested change conflicts with room constraints.
               </p>
               <div className="space-y-3">
                 <div className="space-y-1.5 bg-slate-950/20 p-3 rounded-lg">
                   {log.metadata.warnings.map((w: string, i: number) => (
                     <p key={i} className="text-xs text-amber-200/90 flex items-start gap-2">
                 ...
                 {log.metadata.alternative_suggestion && onAlternativeClick && (
                    <div className="pt-2">
                      <p className="text-xs font-bold text-amber-400/80 uppercase mb-2 flex items-center gap-1">
                        <Lightbulb className="w-3 h-3" /> Try instead:
                      </p>
                      <button 
                        onClick={() => onAlternativeClick(log.metadata.alternative_suggestion!)}
                        className="w-full flex items-center gap-3 p-3 bg-amber-900/10 rounded-lg border border-amber-500/20 text-left hover:bg-amber-900/20 transition-all group"
                      >
                 ...
                 {log.metadata.canForce && onForceExecute && (
                   <div className="pt-2 border-t border-amber-500/20 mt-2">
                     <button 
                       ...
                       className="flex items-center gap-2 text-amber-500/70 hover:text-amber-400 px-2 py-1.5 rounded-lg text-xs font-bold transition-all w-full justify-center group"
                     >
          ...
          <div className={`text-sm leading-relaxed whitespace-pre-wrap ${log.type === 'success' ? 'text-slate-200 bg-emerald-950/20 p-3 rounded-lg border border-emerald-500/20' : 'text-slate-400'}`}>
            {log.content}
          </div>
        )}
      </div>
